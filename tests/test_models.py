...
def test_create_donation():
    ...
    donations = [
        Donation(data=data["_embedded"]["osdi:donations"][num])
        for num in range(len((data["_embedded"]["osdi:donations"])))
    ]  # noqa
    assert donations[0].id == "3039205h-5c40-4e44-bc9b-ed3985713cc8", donations[0].id
    assert donations[0].next_donation == datetime.datetime(
        2017, 10, 29, 14, 54, 26
    ), donations[
        0
    ].next_donation  # noqa
...
def test_create_tag():
    with open("test_data/tags.json") as f:
        data = json.loads(f.read())
    tags = [
        Tag(data=data["_embedded"]["osdi:tags"][num])
        for num in range(len((data["_embedded"]["osdi:tags"])))
    ]  # noqa
